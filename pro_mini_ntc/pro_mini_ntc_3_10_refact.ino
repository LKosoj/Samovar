// Версия 3.10b от 05.05.25
 ...
 int8_t Pressure_enable[4] = {0, 0, 0 ,0};   // Признаки наличия датчиков давления, вычисляемый, для XGZP6897D = 1 (установится если откликнется по I2C),
                              // для MPX10DP = 2 (установится если сохранено в WEB интерфейсе), для HX710B =3 (установится если стоит перемычка с D7 на массу)
 float pressure[4], temperature[4];  // переменные для чтения датчиков давления
//                                          Датчик XGZP6897D, подключенный к I2C №1
 XGZP6897D XGZP(512);  // Объект датчика давления с делителем К=512, делитель зависит от диапазона датчика, искать в даташите, мой (0 - 10 Pa)
...
bool pressure_sensors_enabled[3] = {false, false, false};
