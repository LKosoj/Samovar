...
// note: this FN calls sendBit() & recvBit() but doesn't handle ets_intr_unlock -> calling FN must do this
...
// NOTE: if called separately you need to handle ets_intr_unlock, should be disabled during this FN
...
{
    ets_intr_lock(); // will be enabled at the end of function
    DIRECT_WRITE_LOW(pin_baseReg, pin_bitMask);
    DIRECT_MODE_INPUT(pin_baseReg, pin_bitMask);
