...
float DeltaPress = 0;     // Dranek: Поправка для датчика давления, равна его среднему значению при отсутствии давления и нулевой поправке, её надо изменить на свою
const int buttonPin = 12; // кнопка сброса дрейфа нуля датчика давления
...
void ReadPressure() {                                     //Dranek: Чтение датчика давления и, если есть 1Wire связь, запись его в хаб
  ...
  if (digitalRead(buttonPin)==LOW) DeltaPress = pressure * 0.00750063755419211; // Убираем дрейф нуля датчика давления по замыканию пина D12 на минус
  Temp[0] = pressure * 0.00750063755419211 - DeltaPress;  //Dranek: перевод Паскалей в мм.р.ст и применение поправки
    ...
    int16_t tempCompute = int16_t(Temp[0] * 256);  //Dranek: запись значения давления в качестве параметра температуры по 1Ware
...
void setup() {
  ...
  if (readADS(0, 1) < 2000) { // Если контакты первого термистора замкнуты накоротко включаем режим измерения сопротивлений
    Mode_R = 1;
    ASOled.clearDisplay();
    return (0);
  }
  ...
  if (Pressure_enable) {               // Dranek: Добавляем в хаб 1Ware датчик давления
    ...
    ntcEn[0] = 0;
    ASOled.printString_12x16(F("Тв= "), 0, 0);
  }  // Dranek: если датчика давления нет заменяем его вывод на дисплей на 4 датчик температуры
  ...
  if (readADS(7, 1) < ADS_Trg) {
    hub.attach(ds18b8);
    ntcEn[8] = 1;
    ds18b8.setTemperature((float)25.0);
  } else ntcEn[8] = 0;
...
void loop() {
  if (!Mode_R) { //Если не режим измерения сопротивления
    ...
    if (millis() > (timer + 1000)) {
      ...
      float Rizm = 1 / (1 / R10_izm - 1 / R10); //Сопротивление термистора
      

      ASOled.printString_12x16(F("ADS"), 0, 0);
      dtostrf(float(ADS), 6, 0, outstr);
      ASOled.printString_12x16(outstr, 54, 0);

      ASOled.printString_12x16(F("Uacp"), 0, 2);
      dtostrf(float(Uacp), 6, 3, outstr);
      ASOled.printString_12x16(outstr, 54, 2);

      ASOled.printString_12x16(F("R|| "), 0, 4);
      dtostrf(float(R10_izm), 6, 3, outstr);
      ASOled.printString_12x16(outstr, 54, 4);

      ASOled.printString_12x16(F("Rizm"), 0, 6);
      dtostrf(float(Rizm), 6, 3, outstr);
      if (Rizm > 800) ASOled.printString_12x16("  -No-", 54, 6); // В случае отсутствия термистора выводим заглушку
      else ASOled.printString_12x16(outstr, 54, 6);
    }
  }
}
