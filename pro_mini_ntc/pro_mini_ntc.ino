...
//------------------------------------------Значения для режима килоомметра (включается при зажатой во время загрузки кнопкой действия, либо закороченном первом термисторе)
const float U33 = 3.33;    // * точное напряжение на пине 3.3 V
const float Uref = 2.048;  // * опорное напряжение, точное напряжение на пине A0 ADS1115 без подключенного делителя
const float R62 = 6.200;   // * точное значение 6.2 кОм резистора
const float R10 = 9.990;   // * точное значение 10 кОм резистора
...
#define ASOled LD  // Используем уже созданный в библиотеке дисплея объект LD
const int buttonPin = 12;      // кнопка действия: на 1 экране отображения сброс дрейфа нуля датчика давления
const int button2Pin = 10;     // кнопка переключения экранов отображения
...
void ReadPressure() {      // Чтение датчиков давления и запись их в 1Ware хаб, если есть подключение
  ...
  mysensor.readSensor(temperature, pressure);    //чтение датчика 
  if ((digitalRead(buttonPin) == LOW) && (DispMode == 0)) DeltaPress = pressure * 0.00750063755419211;  // Убираем дрейф нуля датчика давления по замыканию пина D12 на минус если включен 1 экран
  Temp[0] = (float)pressure * 0.00750063755419211 - DeltaPress;   }      //перевод Паскалей в мм.р.ст и применение поправки дрейфа нуля
 ...
 if ((digitalRead(buttonPin) == LOW) && (DispMode == 0)) DeltaPress = (float)pressure;  // Убираем дрейф нуля датчика давления по замыканию пина D12 на минус если включен 1 экран
     pressure =  (float)(((int32_t)readADS(7, 16)  - PressureBaseADS) * PressureQ );    //чтение датчика и перевод значения в мм.рт.ст
...
void disp() {  // Вывод на дисплей
  ...
  if (digitalRead(button2Pin) == LOW) {DispMode++; if (DispMode==3) DispMode=0; ASOled.clearDisplay(); } // Прокрутка режимов дисплея по кнопке 2
  if (DispMode == 0) {
  if (Pressure_enable)  ASOled.printString_12x16(F("Pк= "), 0, 0); else ASOled.printString_12x16(F("Тв= "), 0, 0); //Печатаем заголовки
  ASOled.printString_12x16(F("Tп= "), 0, 2);
  ASOled.printString_12x16(F("Tц= "), 0, 4);
  ASOled.printString_12x16(F("Tк= "), 0, 6);
   for (int i = 0; i < 4; i++) {
    if (ntcEn[i]) {
      dtostrf((float)Temp[i], 6, 2, outstr);  //// ИМХО, хоть и через ж., но так лучше
      ASOled.printString_12x16(outstr, 40, i + i);
    } else ASOled.printString_12x16("  ----  ", 40, i + i);
  } }  
  if (DispMode == 1) {                   // Экран температур с 4 по 7
  ASOled.printString_12x16(F("Тв="), 0, 0);  //Печатаем заголовки
  ASOled.printString_12x16(F("TСА"), 0, 2);
  ASOled.printString_12x16(F("Т6="), 0, 4);
  ASOled.printString_12x16(F("Т7= "), 0, 6);
   for (int i = 0; i < 4; i++) {
    if (ntcEn[i+4]) {
      dtostrf((float)Temp[i+4], 6, 2, outstr);  
      ASOled.printString_12x16(outstr, 40, i + i);
    } else ASOled.printString_12x16("  ----  ", 40, i + i);
  }
...
void setup() {
  ...
  if (Pressure_enable) {               // Добавляем в хаб 1Ware датчик давления
    ...
    ntcEn[0] = 0;
  }  // если датчика давления нет заменяем его вывод на дисплей на 4 датчик температуры
  ...
  if ((readADS(6, 1) < ADS_Trg) && (KTemp == 0)) { // Затействован под температурную компенсацию если коэфф. компенсации не равен 0
    hub.attach(ds18b7);
    ntcEn[7] = 1;
    ds18b7.setTemperature((float)25.0);
  } else ntcEn[7] = 0; 
  if ((readADS(7, 1) < ADS_Trg) && (!(Pressure_enable ==2))) { //Выделен под датчик MPX10DP
    hub.attach(ds18b8); 
    ntcEn[8] = 1;
    ds18b8.setTemperature((float)0.0);
  } else ntcEn[8] = 0;
  if (!Pressure_enable) ntcEn[0] = ntcEn[4];  // в случае отсутствия датчика давления подмена его на 4 термометр
#if defined(SERIAL_DEBUG) || defined(SERIAL_TEST_NTC)
...
void loop() {
  if (!Mode_R) {  //Если не режим измерения сопротивления
    ...
    if (hub.startConvert && millisConvert < millis()) {
      if (Pressure_enable) ReadPressure();      // Чтение давления
      uint16_t ad_temp;
      int16_t tempCompute;
      ...
      if (!Pressure_enable) Temp[0] = Temp[4];  // в случае отсутствия датчика давления подмена его на 4 термометр
    ...
    if (millis() > (timer + 1000)) {
      ...
      float Rizm;
      if (digitalRead(button2Pin) == LOW) { // Меняем канал измерений если нажата кнопка
        ADSCh++;
        if (ADSCh == 8) ADSCh = 0;
      }
      ...
      ASOled.printNumber(long(ADSCh+1), 36, 0);
      ...
      if (Rizm > 100) ASOled.printString_12x16("  -No-", 54, 6);  // Вне диапазона, выводим заглушку
