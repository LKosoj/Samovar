...
SimpleStringQueue — Очередь строк для асинхронной отправки сообщений
...
Помещает сообщение в очередь на отправку внешним сервисам (Blynk и др.)
Аргументы
msg — Текст сообщения
...
Аргументы
— — Устанавливает флаг `ota_running`, отправляет событие о начале обновления, отключает сторонние сервисы (Blynk)
Возвращает
void
Исключения
Нет
...
Отправляет текстовое сообщение указанного типа во внешние сервисы (Blynk)
Аргументы
m — текст сообщения
