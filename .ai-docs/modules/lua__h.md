...
Исключения
Функция ставит команду через `queue_samovar_command()`; если очередь недоступна или занята, возвращает ошибку Lua
...
Исключения
При выключении питания может поставить reset-команду через `queue_samovar_reset_command()`
...
Выполняет переданную строку как Lua-скрипт с логированием.
Аргументы
