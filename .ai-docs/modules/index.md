...
- `SendMsg(const String& m, MESSAGE_TYPE msg_type)` — Отправляет сообщение пользователю через доступные интерфейсы (например, Blynk, дисплей).
