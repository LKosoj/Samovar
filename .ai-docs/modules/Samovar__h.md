...
---
SamovarCommandMsg / samovar command queue
Статическая FreeRTOS-очередь команд для синхронизации между задачами и основным циклом
Аргументы
command — Значение `SamovarCommands` (например, запуск, остановка, калибровка), переданное из интерфейса, Blynk, Lua, меню или аварийного обработчика
Возвращает
Результат постановки команды в очередь через `queue_samovar_command()` или `queue_samovar_reset_command()`
Исключения
Нет
