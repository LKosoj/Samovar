...
Модуль реализует логику управления самоваром для пивоварения, включая контроль температуры, управление нагревателем по ПИД-регулятору, обработку программ затирания, контроль кипения, взаимодействие с периферией (насос, клапаны, мешалка, шаговый двигатель). Поддерживает сохранение профилей, автотюнинг ПИД, обработку ошибок питания и отправку уведомлений. Основное состояние процесса управляется через программу затирания с переходами между этапами.
...
msg — true, если нужно отправить уведомление через Blynk
Возвращает
Нет
Исключения
Нет
...
Отправить сообщение пользователю через доступные интерфейсы (Blynk, дисплей и т.п.).
Аргументы
m — Текст сообщения
