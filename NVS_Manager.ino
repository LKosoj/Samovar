...
static bool decode_setup_payload(
  ...
  SetupEEPROM decoded{};
  CanonicalProfileReader<SAMOVAR_PROFILE_PAYLOAD_SIZE_V10> reader(payload);
...
static bool decode_setup_payload_v9(
    const uint8_t* payload,
    SetupEEPROM& candidate) {
  SetupEEPROM decoded{};
  CanonicalProfileReader<SAMOVAR_PROFILE_PAYLOAD_SIZE_V9> reader(payload);
  if (!decode_setup_payload_fields<false, false>(reader, decoded) ||
      !decode_setup_payload_v2only_fields(reader, decoded) ||
      !decode_setup_payload_v3only_fields(reader, decoded) ||
      !decode_setup_payload_v4only_fields<false, false>(reader, decoded) ||
      !decode_setup_payload_v9only_fields(reader, decoded) ||
      reader.size() != SAMOVAR_PROFILE_CANONICAL_BYTES_V9 ||
      !reader.finish()) return false;
  set_profile_version_defaults(decoded, 9);
  candidate = decoded;
  return true;
}

static bool decode_setup_payload_v8(
    const uint8_t* payload,
    SetupEEPROM& candidate) {
  SetupEEPROM decoded{};
  ...
  if (!decode_setup_payload_fields<false, false>(reader, decoded) ||
      ...
      !reader.finish()) return false;
  set_profile_version_defaults(decoded, 8);
  candidate = decoded;
  return true;
}
...
static bool decode_setup_payload_v7(
  ...
  if (!decode_setup_payload_fields<false, true>(reader, decoded) ||
      ...
      !decode_setup_payload_v3only_fields(reader, decoded) ||
      !decode_setup_payload_v4only_fields<false, false>(reader, decoded) ||
      reader.size() != SAMOVAR_PROFILE_CANONICAL_BYTES_V7 ||
      !reader.finish()) return false;
  candidate = decoded;
  return true;
}

static bool decode_setup_payload_v6(
    const uint8_t* payload,
    SetupEEPROM& candidate) {
  SetupEEPROM decoded{};
  ...
  if (!decode_setup_payload_fields<false, true>(reader, decoded) ||
      ...
      reader.size() != SAMOVAR_PROFILE_CANONICAL_BYTES_V6 ||
      !reader.finish()) return false;
  candidate = decoded;
  return true;
}

static bool decode_setup_payload_v5(
    const uint8_t* payload,
    SetupEEPROM& candidate) {
  SetupEEPROM decoded{};
  ...
  if (!decode_setup_payload_fields<false, true>(reader, decoded) ||
      ...
      !reader.finish()) return false;
  set_profile_version_defaults(decoded, 5);
  candidate = decoded;
  return true;
}
...
#define SAMOVAR_DEFAULT_TERM_ALL(deflt)
...
#undef SAMOVAR_DEFAULT_TERM_ALL
}

static bool decode_setup_payload_v3(
    const uint8_t* payload,
    SetupEEPROM& candidate) {
  SetupEEPROM decoded{};
  ...
  if (!decode_setup_payload_fields<true, true>(reader, decoded) ||
      ...
      !reader.finish()) return false;
  set_profile_version_defaults(decoded, 3);
  candidate = decoded;
  return true;
}

static bool decode_setup_payload_v2(
    const uint8_t* payload,
    SetupEEPROM& candidate) {
  SetupEEPROM decoded{};
  ...
  if (!decode_setup_payload_fields<true, true>(reader, decoded) ||
      ...
      !reader.finish()) return false;
  set_profile_version_defaults(decoded, 2);
  candidate = decoded;
  return true;
}
...
ProfileLoadResult load_profile_nvs(SetupEEPROM& candidate, PersistResult& persistResult) {
  ...
  if (storedSize == V9ProfileCodec::BLOB_SIZE) {
    V9ProfileCodec::Blob encoded{};
    size_t readSize = V9ProfileCodec::BLOB_SIZE;
    const uint8_t readResult = nvs_read_blob(
        readHandle, SAMOVAR_PROFILE_KEY, encoded.bytes, readSize);
    nvs_close(readHandle);
    if (readResult != PROFILE_VALUE_FOUND) return PROFILE_LOAD_READ_FAILED;
    if (readSize != V9ProfileCodec::BLOB_SIZE) return PROFILE_LOAD_SHORT_READ;

    uint8_t payload[V9ProfileCodec::PAYLOAD_SIZE] = {};
    const ProfileLoadResult validation = load_codec_result(V9ProfileCodec::decode(
        encoded.bytes, V9ProfileCodec::BLOB_SIZE, payload));
    if (validation != PROFILE_LOAD_OK) return validation;
    SetupEEPROM migrated{};
    if (!decode_setup_payload_v9(payload, migrated)) return PROFILE_LOAD_PAYLOAD_ENCODING;
    candidate = migrated;
  ...
  if (storedSize == V8ProfileCodec::BLOB_SIZE) {
    V8ProfileCodec::Blob encoded{};
    size_t readSize = V8ProfileCodec::BLOB_SIZE;
    const uint8_t readResult = nvs_read_blob(
        readHandle, SAMOVAR_PROFILE_KEY, encoded.bytes, readSize);
    nvs_close(readHandle);
    if (readResult != PROFILE_VALUE_FOUND) return PROFILE_LOAD_READ_FAILED;
    if (readSize != V8ProfileCodec::BLOB_SIZE) return PROFILE_LOAD_SHORT_READ;

    uint8_t payload[V8ProfileCodec::PAYLOAD_SIZE] = {};
    const ProfileLoadResult validation = load_codec_result(V8ProfileCodec::decode(
        encoded.bytes, V8ProfileCodec::BLOB_SIZE, payload));
    if (validation != PROFILE_LOAD_OK) return validation;
    SetupEEPROM migrated{};
    if (!decode_setup_payload_v8(payload, migrated)) return PROFILE_LOAD_PAYLOAD_ENCODING;
    candidate = migrated;
    persistResult = save_profile_nvs(migrated);
    if (persistResult != PERSIST_OK) return PROFILE_LOAD_MIGRATION_PERSIST_FAILED;
    return PROFILE_LOAD_OK;
  ...
  if (storedSize == V6ProfileCodec::BLOB_SIZE) {
    V6ProfileCodec::Blob encoded{};
    size_t readSize = V6ProfileCodec::BLOB_SIZE;
    const uint8_t readResult = nvs_read_blob(
        readHandle, SAMOVAR_PROFILE_KEY, encoded.bytes, readSize);
    nvs_close(readHandle);
    if (readResult != PROFILE_VALUE_FOUND) return PROFILE_LOAD_READ_FAILED;
    if (readSize != V6ProfileCodec::BLOB_SIZE) return PROFILE_LOAD_SHORT_READ;

    uint8_t payload[V6ProfileCodec::PAYLOAD_SIZE] = {};
    const ProfileLoadResult validation = load_codec_result(V6ProfileCodec::decode(
        encoded.bytes, V6ProfileCodec::BLOB_SIZE, payload));
    if (validation != PROFILE_LOAD_OK) return validation;
    SetupEEPROM migrated{};
  ...
  if (storedSize == V5ProfileCodec::BLOB_SIZE) {
    V5ProfileCodec::Blob encoded{};
    size_t readSize = V5ProfileCodec::BLOB_SIZE;
    const uint8_t readResult = nvs_read_blob(
        readHandle, SAMOVAR_PROFILE_KEY, encoded.bytes, readSize);
    nvs_close(readHandle);
    if (readResult != PROFILE_VALUE_FOUND) return PROFILE_LOAD_READ_FAILED;
    if (readSize != V5ProfileCodec::BLOB_SIZE) return PROFILE_LOAD_SHORT_READ;

    uint8_t payload[V5ProfileCodec::PAYLOAD_SIZE] = {};
    const ProfileLoadResult validation = load_codec_result(V5ProfileCodec::decode(
        encoded.bytes, V5ProfileCodec::BLOB_SIZE, payload));
    if (validation != PROFILE_LOAD_OK) return validation;
    SetupEEPROM migrated{};
  ...
  if (storedSize == V4ProfileCodec::BLOB_SIZE) {
    V4ProfileCodec::Blob encoded{};
    size_t readSize = V4ProfileCodec::BLOB_SIZE;
    const uint8_t readResult = nvs_read_blob(
        readHandle, SAMOVAR_PROFILE_KEY, encoded.bytes, readSize);
    nvs_close(readHandle);
    if (readResult != PROFILE_VALUE_FOUND) return PROFILE_LOAD_READ_FAILED;
    if (readSize != V4ProfileCodec::BLOB_SIZE) return PROFILE_LOAD_SHORT_READ;

    uint8_t payload[V4ProfileCodec::PAYLOAD_SIZE] = {};
    const ProfileLoadResult validation = load_codec_result(V4ProfileCodec::decode(
        encoded.bytes, V4ProfileCodec::BLOB_SIZE, payload));
    if (validation != PROFILE_LOAD_OK) return validation;
    SetupEEPROM migrated{};
  ...
  if (storedSize == V2ProfileCodec::BLOB_SIZE) {
    V2ProfileCodec::Blob encoded{};
    size_t readSize = V2ProfileCodec::BLOB_SIZE;
    const uint8_t readResult = nvs_read_blob(
        readHandle, SAMOVAR_PROFILE_KEY, encoded.bytes, readSize);
    nvs_close(readHandle);
    if (readResult != PROFILE_VALUE_FOUND) return PROFILE_LOAD_READ_FAILED;
    if (readSize != V2ProfileCodec::BLOB_SIZE) return PROFILE_LOAD_SHORT_READ;

    uint8_t payload[V2ProfileCodec::PAYLOAD_SIZE] = {};
    const ProfileLoadResult validation = load_codec_result(V2ProfileCodec::decode(
        encoded.bytes, V2ProfileCodec::BLOB_SIZE, payload));
    if (validation != PROFILE_LOAD_OK) return validation;
    SetupEEPROM migrated{};
...
struct LegacyEepromLayout {
  ...
  // V3ONLY/V4ONLY по X-макросу); retired UPTO4/UPTO5/UPTO6 некуда девать - не копируются.
