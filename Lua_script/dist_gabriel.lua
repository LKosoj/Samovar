...
TankTemp = getNumVariable("TankTemp") + 0 --получаем текущую температуру куба
...
if (sg == 0) then
  ...
  sendMsg("Начата дистилляция по Габриелю!", -1) --пишем в консоль браузера
...
if (gb == 0 and b_temp > 0) then
  setObject("gb", 1)
  sendMsg("Начат отбор в емкость №0!", -1) --пишем в консоль браузера
