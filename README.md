...
3. Мы должны понимать, какую крепость спирта обеспечит колонна при отборе тела.
4. Мы понимаем примерный объем тела, его надо отобрать, не захватив головы (решается выбором процентного соотношения голов к общему объему, мнение может быть только экспертное), и не захватив хвосты (считается как разнность общего объема с объемом спирта и голов), это контролируется предотвращением роста температуры в царге и голове, а так же за счет управления скоростью отбора.
5. Отсекаем головы и хвосты, получаем тело. Если делаем из сахара - на этом отбор закончен.
6. Если мы отбираем что-то ароматное - головы и хвосты надо разделить на несколько емкостей. Всего Самовар может разделить на 11 емкостей, но можно использовать до 20, только надо будет заменить приемные емкости. Процентное разделение настраивается в программе отбора. Определяется экспертно.
7. После завершения отбора тело будет того качества, которое может дать колонна, а в других емкостях будет псевдоразделение по фракциям (псевдо потому, что там будут не чистые фракции, а их смесь).
8. Решаем, что делать с этими фракциями. Выпить, вылить, поставить в бочку, или на щепу.
