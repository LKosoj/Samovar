...
Во время активного процесса редактор `/edit` запрещает операции, меняющие
файловую систему: `DELETE`, создание файла через `PUT` и запись каждого чанка
загрузки. Просмотр списка и скачивание файлов остаются доступны. После остановки
процесса изменение файлов разрешается только после фактического закрытия журнала.
