...
`data_raw/` — исходники интерфейса; `python3 tools/build_web_assets.py` собирает из
них каталог `data/` для LittleFS. Рабочие страницы загружаются как статические gzip-
файлы и до первого опроса получают начальные данные одним запросом `/ui-bootstrap`.
Единственное исключение — `setup.htm`: она остаётся несжатой шаблонной страницей.
...
### Лог процесса и сессии (Blynk)

Лог процесса, старт сессии и сообщения уходят на сервер не по MQTT (убран целиком),
а через уже открытое TLS-соединение с Blynk: виртуальные пины V34 (строка лога),
V35 (старт сессии) и V26 (сообщения). Период записи лога фиксирован —
`LOG_PERIOD_S = 4` секунды, настройки в веб-интерфейсе для этого больше нет. В
простое (нет активного процесса) V34 всё равно уходит раз в 5 секунд с текущими
температурами, чтобы приложения видели показания и без запущенного процесса.

Если процесс прервался аварийной перезагрузкой и его же программу запускают заново
в течение 30 минут, прошивка не открывает новую сессию, а продолжает прежнюю (тот же
`sessionId`, признак `resume=1` в V35) — сервер дописывает существующую запись лога,
а не создаёт новую. Подробный формат V34/V35 — в `SamovarServer/SamovarMobile/PIN_SPEC.md`
(§11), серверная часть — в `SamovarServer/CLAUDE.md`.
...
| **7. Интеграции (IoT)** | **Встроенные.** Blynk "из коробки". | **Отсутствуют.** | **Редко.** Обычно это проприетарные протоколы или ModBus для связи с ПК. | **Стандарт.** OPC-UA, ModBus, Profinet и др. промышленные протоколы. |
...
| **5. Интеграции и IoT** | **Встроенные.** Blynk "из коробки". | **Ограниченные.** Интеграция с iSpindel, в некоторых форках есть MQTT. | **Широкие (через плагины).** MQTT, iSpindel, оповещения и многое другое. | **Практически отсутствуют.** |
