...
//Адаптивный PI для нагрева в режимах Пиво/Сыр (насос охлаждения всегда на обычном PID).
//Задайте 0, чтобы использовать прежние PID-алгоритмы.
...
//Температура завершения перегонки по умолчанию
...
#define NBK_WORK_PRESSURE_RATIO 0.5f // [T1-2026-09-03] рабочий потолок давления = доля от давления захлёба (40 мм -> 20 мм)
#define NBK_PRESSURE_MARGIN 5 // [T1-2026-09-03] мм рт.ст. ниже давления в момент захлёба при обучении потолка
#ifdef SAMOVAR_USE_SEM_AVR
#define NBK_DEFAULT_PROGRAM "H;1;0\nS;10;2000\nO;0;0\nW;0;0\n" //программа НБК по-умолчанию (ватты)
#else
#define NBK_DEFAULT_PROGRAM "H;1;0\nS;10;167\nO;0;0\nW;0;0\n" //программа (вольты, для ТЭН 14 Ом)
#endif
// #define USE_NBK_DELTA_PRESSURE // Включение коррекции температуры барды по давлению в бардоотводчике
// #define USE_NBK_END_BY_STEAM_RISE // Доп. признак конца браги: завершать Работу НБК, когда Тп выросла на NBK_END_STEAM_RISE относительно минимума, зафиксированного в Работе, дольше 2*Ин
#define NBK_END_STEAM_RISE 5.0f // порог роста Тп (°C) для USE_NBK_END_BY_STEAM_RISE
//**************************************************************************************************************
...
#define WATER_FLOW_MIN_PULSES 7              //Проток считается реальным, если импульсов за такт не меньше этого порога

#define USE_HEAD_LEVEL_SENSOR                //использовать датчик уровня флегмы в голове (датчик P-N-P). Если используется - в случае срабатывания датчика снизит мощность на 1*PWR_FACTOR (для KVIC: 1 В, для RMVK: 2 В; для SEM_AVR — на 3% от текущего)
...
//#define USE_ADS1115 0x48                     //использовать ADS1115: датчик pH автоматически подключается к AIN0
...
#define PROGRAM_DONE_AUTO_POWEROFF_MIN 30  //время (мин) удержания статуса "Выполнение программы завершено" - режим "работа на себя": насос остановлен, нагрев продолжается, оператор может продолжить отбор; по истечении - автоотключение. 0 = отключать немедленно
#endif
