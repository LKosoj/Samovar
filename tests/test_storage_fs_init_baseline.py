...
class StorageFsInitBaselineParityTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.current_header = FS_INIT_HEADER.read_text(encoding="utf-8")
        cls.current_server_init = SERVER_INIT_HEADER.read_text(encoding="utf-8")
        cls.baseline_fs = _read_git_file(BASELINE_FS_COMMIT, "FS.ino")

    def test_fs_init_matches_pre_split_baseline(self) -> None:
        current_body = _normalize_cpp_body(
            _extract_function_body(self.current_header, "void FS_init(void)")
        )
        baseline_body = _normalize_cpp_body(
            _extract_function_body(self.baseline_fs, "void FS_init(void)")
        )
