...
void stop_razgon(void) {  //===========Подпрограммка остановки режима "Разгон"================
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { // S-26: RMW битовых полей fl; ISR(TIMER2) читает fl.razg_on/fl.TRelay
...
void set_newPDM(uint16_t power) { // Функция установки текущей мощности по запросу
  uint16_t new_PDMust;
  if (!Pnom) {          // Если Pnom не откалиброван (равен нулю) - безопасно выключаем нагрев
    new_PDMust = 0;
  }
  else if (power >= Pnom) {  // Если параметр превышает максимально возможное значение,
    new_PDMust = CICLE; // значит ставим максимально возможное значение.
  }
  else {
...
//
        ...
        default: {  //=============А если не в меню, то...
          switch (butt) {
            case 1:
              ...
              break;
            case 2:
              // RMW PDMust — те же условия защиты, что case 1 (см. выше).
            ...
            case 4:
              if (PDMust == 0) {    //Если мы не в меню и мощность ТЭНа нулевая, то...
                ...
                PDMust = 0;                   // Экстренно выключим ТЭН (защита — см. case 1)
  ...
  if (pin_STAB_OFF_STATE && !fl.stab_off) { // Если есть сигнал аварийного останова
    if (PDMust) { // Если уставка ненулевая... (чтение безопасно — см. case 1 выше)
      remember_last_power_setting();// Запомним последнюю уставку
      PDMust = 0;                   // Экстренно выключим ТЭН (запись безопасна — см. case 1)
...
//
  ...
  //
  //pp_Delay(10);   // Подождем 20 полупериодов
//
#ifdef DisplayReset
...
//
  if (fl.sum) { //==========Обработка данных от АЦП и корректировка выдаваемой мощности============
    // S-27: атомарно захватываем разделяемые ISR(ADC) переменные до расчёта
    uint32_t l_U_sum;
    uint16_t l_sc_sum;
    uint16_t l_PDMust;
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
      l_U_sum  = U_sum;
      l_sc_sum = sc_sum;
      l_PDMust = PDMust; // S-28: атомарно читаем PDMust (пишется loop, читается ISR TIMER2)
    }
#ifdef NOT_LM358
    // 0,55 - Коэффициент нормирования ((380/512)^2, 380В максимальное амплитудное) для Rail-to-Rail операционника
    l_U_sum /= l_sc_sum;    //Ненормированный квадрат среднеквадратичного
    l_U_sum *= 0.55;        //Нормированный квадрат среднеквадратичного
#else
...
//    tmp /= 2;
    ...
    if (tmp > CICLE || fl.razg) { // Следим, чтобы pdm не превышала CICLE
      ...
      ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
        fl.Ulow = 0;
        pdm = tmp; // S-28: атомарно пишем 16-бит pdm
      }
    }

    // Проверяем величину напряжения
    l_U_sum *= (long)400;             // Произведем некоторое математическое колдунство,
    l_U_sum = sqrt(l_U_sum);         // чтобы получить один знак после запятой без float
    l_U_sum++; l_U_sum /= 2;        // и с правильным округлением.
    U_real_dec = l_U_sum % 10;       // Среднеквадратичное (дробная часть)
    U_real = l_U_sum / 10;           // Среднеквадратичное (целая часть)
    //
    // Контролируем значение
    if ( U_real < U_MIN ) { //Действующее напряжение сети ниже U_MIN - отключим ТЭН (авария)
      // S-26/28: атомарно пишем fl.Udown и pdm
      ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
