...
NORMALIZED_SHA256 = {
    ...
    # не выходили разной ширины; добавлен класс .nowrap для диапазона плотности
    ...
    # 04.09.2026: при обрыве связи гасится и блокируется вся страница (html.connection-lost).
    # 04.09.2026: скрытый file-input больше не class=button; нативный picker спрятан,
    # подпись загрузки не сдвигается от .button:hover { top:1px }.
    # 04.09.2026: колонка .ingDetail и .mashWater на карточке рецепта brewxml.htm
    # (геометрия списка ингредиентов/пауз, палитра та же).
    ...
    # 06.09.2026: скрытые вкладки устройств действительно скрываются через
    # .tab input[hidden]; правило меняет только видимость, а не палитру.
    # 06.09.2026: комментарий .nowrap приведён к диапазону PackDens 0-100;
    # исполняемые CSS-правила не изменились.
    "style.css": "3c2ddcbd9390f04a4cfc29f0ae088f32d805ed4add44921526d04d5b261a37e5",
    ...
    # 04.09.2026: порядок варки передаётся в app.js из серверной настройки.
    # Цветов и разметки не добавлялось.
    ...
    # setup.htm: клиентская нижняя граница StepperStepMl изменилась с 0 на 1;
    # цветов/разметки не касалось (см. tools/smoke_u04_responsive_layout.py
    ...
    # 04.09.2026: Су-вид и Lua убраны из списка выбора режима (скрытые option
    # оставлены, чтобы сохранённый Mode=5/6 не затирался). Цветов не касается.
    # 04.09.2026: поле BeerBrewOrder и его подсказка добавлены на вкладку Beer.
    # Палитра и инлайновые цвета не менялись.
    # 05.09.2026: поля MPX, второго I2C-насоса и сервопривода НБК с подсказками.
    # 05.09.2026: вкладка Cheese и предупреждения об операторском выборе
    # PH/MPX на LUA_PIN и клапана/ТЭНа на реле 4; новых цветов нет.
    # 06.09.2026: PackDens допускает весь диапазон 0-100; менялись только
    # числовая граница и подпись диапазона.
    "setup.htm": "9be391b6e601e9488487e71377fb38ec4714536e0b1b2d7448bd9e3cf2f36c0e",
    ...
    # Цветов не касается.
    # 06.09.2026: скорости загруженной программы меняются только по явному
    # нажатию кнопки рекомендаций. Удалены три автоматических вызова; цветов нет.
    "program.htm": "a9e41c558aa9c4f12de21893809a1f5b1f13ff4f2f2624a25212ce7f4b3b0337",
    ...
    # 04.09.2026: loadCsv сам повторяет HTTP 503 BUSY, палитра рядов та же.
    "chart.js": "ca5b1f8143474a067484d1782a55efae65e66c73a071f1123fab87e67b95ac59",
}
FROZEN_SHA256 = {
