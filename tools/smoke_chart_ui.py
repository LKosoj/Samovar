...
CHART_HTM = ROOT / "data" / "chart.htm"
CHART_JS = ROOT / "data" / "chart.js"
...
if chart_htm:
    for token in ["cdn.amcharts.com", "am4core", "am4charts", "am4themes"]:
        if token in chart_htm:
            errors.append(f"data/chart.htm still depends on external/amCharts token: {token}")
    for token in ["reloadFrequency", "dataSource", "chart.dataSource"]:
        if token in chart_htm:
            errors.append(f"data/chart.htm contains forbidden full-reload chart token: {token}")

    if '<script src="chart.js"></script>' not in chart_htm:
        errors.append("data/chart.htm does not load local chart.js")
    for token in [
        "function initChart ()",
        ...
        if token not in chart_htm:
            errors.append(f"data/chart.htm missing W7 chart token: {token}")

    try:
        dom_body = extract_function_body(chart_htm, "document.addEventListener('DOMContentLoaded', function ()")
        init_pos = dom_body.find("initChart();")
        poll_pos = dom_body.find("startPolling();")
        if init_pos == -1 or poll_pos == -1:
            errors.append("data/chart.htm DOMContentLoaded does not call initChart() and startPolling()")
        elif poll_pos < init_pos:
            errors.append("data/chart.htm starts polling before chart init is attempted")
    ...
    try:
        ...
        for token in ["chartBlock.textContent", "try {", "catch (err)", "chart.loadCsv('data.csv').catch"]:
            if token not in init_body:
                errors.append(f"data/chart.htm initChart() does not fail explicitly without blocking polling: {token}")
    except ValueError as exc:
        errors.append(str(exc))
    ...
    try:
        ...
        for token in ["chartLastAppendMs", "15000", "chart.appendAjaxPoint(myObj)"]:
            if token not in append_body:
                errors.append(f"data/chart.htm appendChartPoint() does not enforce throttled ajax append: {token}")
    except ValueError as exc:
        errors.append(str(exc))
    ...
    try:
        ...
        if "loadCsv" in refresh_body or "data.csv" in refresh_body:
            errors.append("data/chart.htm refresh_chart() reloads full data.csv")
        if "chart.setAutoRefresh" not in refresh_body:
            errors.append("data/chart.htm refresh_chart() does not toggle local auto refresh")
    except ValueError as exc:
        errors.append(str(exc))
...
if chart_js:
    for token in ["SamovarChart", "parseCsv(text)", "MAX_RENDER_POINTS = 600", "appendAjaxPoint", "decimate(rows)"]:
        if token not in chart_js:
            errors.append(f"data/chart.js missing local renderer token: {token}")
    for token in ["cdn.amcharts.com", "am4core", "am4charts", "reloadFrequency", "dataSource"]:
        if token in chart_js:
            errors.append(f"data/chart.js contains forbidden chart dependency/reload token: {token}")
    if not re.search(r"fetch\(url,\s*\{\s*cache:\s*['\"]no-store['\"]\s*\}\)", chart_js):
        errors.append("data/chart.js initial CSV load is not explicit no-store fetch")
