...
COMMON_FUNCTIONS = [
  ...
  "sendI2CPump",
  "stopI2CPump",
  # Восстановленное управление насосом живёт только в app.js. Старые имена выше
  # оставлены, чтобы не вернулись дореформенные копии; новые нужны затем же.
  "sendI2cPump",
  "stopI2cPump",
  "run_lua",
  "run_strlua",
...
def check_pages(errors):
  ...
  for page in PAGES:
    ...
    if text.count("SamovarApp.startTelemetryPage(") != 1:
      errors.append(f"{rel} must start exactly one shared telemetry lifecycle")
    if "SamovarApp.pollAjax(" in text or "SamovarApp.startPollLoop(" in text:
      errors.append(f"{rel} retains local telemetry polling ownership")
    ...
    if page.name == "beer.htm" and re.search(r"\bfunction\s+sendpumpspeed\s*\(", text):
      errors.append("data_raw/beer.htm still defines dead sendpumpspeed()")
    if page.name == "nbk.htm":
      if re.search(r"\bfunction\s+sendwaterpwm\s*\(", text):
        errors.append("data_raw/nbk.htm still defines dead sendwaterpwm()")
...
def check_app_js(errors):
  ...
  if not re.search(r"\bconst\s+HISTORY_LIMIT\s*=\s*500\s*;", text):
    errors.append("data_raw/app.js missing const HISTORY_LIMIT = 500")
  ...
  if send_command_body:
    ...
    known_branch_pos = send_command_body.find("if (knownToken && result.ok)")
    http_branch_pos = send_command_body.find("if (!resp.ok)")
    if not known_token_match:
      errors.append("data_raw/app.js sendCommand() does not identify known command tokens")
    elif "token &&" not in known_token_match.group(1):
      errors.append("data_raw/app.js sendCommand() known-token check does not guard empty tokens")
    if known_branch_pos == -1:
      errors.append("data_raw/app.js sendCommand() does not branch on known command tokens")
    if http_branch_pos == -1:
      errors.append("data_raw/app.js sendCommand() missing generic HTTP error fallback")
    if known_branch_pos != -1 and http_branch_pos != -1 and known_branch_pos < http_branch_pos:
      errors.append("data_raw/app.js sendCommand() trusts a success token before checking HTTP status")
    if "addMessage(result.text, result.level);" not in send_command_body:
      errors.append("data_raw/app.js sendCommand() does not render non-OK command tokens with result.level")
    if re.search(
      r"body\s*\|\|\s*\(\s*resp\.ok\s*\?\s*['\"]OK['\"]\s*:\s*['\"]BAD_REQUEST['\"]\s*\)",
      send_command_body,
    ):
      errors.append("data_raw/app.js sendCommand() synthesizes BAD_REQUEST for empty HTTP errors")
  ...
  if play_body and not re.search(r"\.play\s*\(\s*\)[\s\S]*\.catch\s*\(", play_body):
    errors.append("data_raw/app.js playSound() does not catch play() rejection")
  ...
  if unlock_body:
    for token in ["soundPlaying = false;", "if (soundEnabled && alarmActive) playSound(true);"]:
      if token not in unlock_body:
        errors.append(f"data_raw/app.js unlockAudio() missing active-alarm resume token: {token}")

  if "getHistory().map(renderHistoryEntry).join('')" not in text:
    errors.append("data_raw/app.js history render does not use map(...).join('')")
  if "messages.map(renderMessage).join('')" not in text:
    errors.append("data_raw/app.js message render does not use map(...).join('')")
  if re.search(r"\.innerHTML\s*\+=", text):
    errors.append("data_raw/app.js contains innerHTML +=")
  if "function cssVar(name)" not in text or "cssVar: cssVar" not in text:
    errors.append("data_raw/app.js missing shared cssVar() helper/export for theme tokens")
  for token in [
    ...
    if token not in text:
      errors.append(f"data_raw/app.js missing nonblocking notify token: {token}")
  try:
    poll_body = extract_function_body(text, "async function pollAjax(renderFn, sinks)")
  except ValueError as exc:
    errors.append(str(exc))
    poll_body = ""
  if poll_body:
    if "reportUiError(err, activeSinks.message)" not in poll_body:
      errors.append("data_raw/app.js pollAjax() does not report render/event errors through the selected message sink")
    if "setConnectionError" in poll_body:
      errors.append("data_raw/app.js pollAjax() catches DOM render errors through setConnectionError")


def check_theme_tokens_w68(errors):
  style = read_text(STYLE_CSS)
  if style is None:
    errors.append("data_raw/style.css not found")
    return
  ...
  for token in [
    ...
    if token not in style:
      errors.append(f"data_raw/style.css missing W6.8 theme token {token}")
...
def check_nbk_get_program_w69(errors):
  ...
  text = read_text(nbk)
  if text is None:
    errors.append("data_raw/nbk.htm not found")
    return
  ...
  if max_guard_pos == -1 or apply_loop_pos == -1 or apply_loop_pos < max_guard_pos:
    errors.append("data_raw/nbk.htm W6.9 getProgram() applies UI rows before max-line guard")
