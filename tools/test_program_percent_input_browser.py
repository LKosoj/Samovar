...
Правка: set_num() теперь пропускает обрезку СОБСТВЕННОГО значения поля,
пока это поле - document.activeElement (т.е. пользователь ещё печатает в
него), а onchange (уже был у поля) обрезает и фиксирует значение при уходе
фокуса. Поле "процент" держит oninput (это же событие требует существующий
tools/test_numeric_input_ui_browser.py - сводка должна обновляться мгновенно),
поле "скорость" переведено на onchange (сводка обновляется только при уходе
фокуса, а не на каждый символ).
