...
if brewxml:
  if re.search(r"<[A-Za-z][^>]*\bclassName\s*=", brewxml):
    errors.append("data_raw/brewxml.htm still uses React-style className in static HTML")
  if "margin: 20 auto" in brewxml:
    errors.append("data_raw/brewxml.htm still has unitless margin: 20 auto")
  if 'class="ingredients"' not in brewxml:
    errors.append("data_raw/brewxml.htm missing class=\"ingredients\" sections")
  if 'id=\'WProgram\' name=\'WProgram\' style="display: none;"' not in brewxml:
    errors.append("data_raw/brewxml.htm hidden WProgram textarea still reserves layout space")
  if re.search(r"<textarea[^>]+id=['\"]WProgram['\"][^>]+visibility:\s*hidden", brewxml):
    errors.append("data_raw/brewxml.htm WProgram textarea still uses visibility:hidden")
...
if program:
  if "width: 20;" in program:
    errors.append("data_raw/program.htm hidden textarea still has unitless width: 20")
  for token in [
    ...
    if token not in program:
      errors.append(f"data_raw/program.htm missing fixed hidden textarea style: {token}")
