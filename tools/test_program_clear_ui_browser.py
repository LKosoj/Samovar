...
BROWSER_TEST = r'''async page => {
  ...
  const bootstrap = __BOOTSTRAP_FIXTURE__;
  ...
  const ajaxFixture = {
    ...
    bk_water_auto: false, bk_steam_setpoint: 0
  };
  const columnFixture = {
...
def main():
  ...
  try:
    with tempfile.TemporaryDirectory(prefix="samovar-program-clear-browser-") as temp_dir:
      ...
      browser_test = BROWSER_TEST.replace("__BASE_URL__", json.dumps(base_url)).replace(
