...
int main() {
  ...
  // 5. [Б1.1] Приоритет: StepperStepMl==0 И ошибочная строка отбора одновременно ->
  ...
  check(!result5, "StepperStepMl==0 со строкой отбора должен быть отклонён");
  ...
  // Lua-строка не использует насос отбора и не должна требовать его калибровки.
  reset_program();
  SamSetup.StepperStepMl = 0;
  program[0].WType = 'L';
  ProgramLen = 1;
  String errLua;
  check(validate_rect_program_startable(errLua),
        "Lua-only программа не должна требовать калибровку насоса отбора");
