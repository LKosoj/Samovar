...
Чтобы запустить наш воображаемый процесс с помощью ЖК-дисплея, мы будем перемещаться по экранам с помощью энкодера, пока не найдем опцию «Start» (Запустить) или не выберем программу, а затем нажмем кнопку. Это запустит функцию, такую как `menu_samovar_start()`.
...
В нашем случае нажатие кнопки «Пуск» на веб-странице запускает функцию `web_command`, которая ставит команду в очередь (`queue_samovar_command(SAMOVAR_START)`), сообщающую основной логике Samovar о начале процесса.
...
В нашем случае нажатие кнопки «Старт» на веб-странице вызовет функцию `web_command`, которая затем поставит команду в очередь (`queue_samovar_command(SAMOVAR_START)`), указывающую основной логике самовара начать процесс.
  ...
  if (request->params() == 1) { // Предполагаем простую команду с одним параметром
    if (request->hasArg(«start») && PowerOn) {
      ...
      if (Samovar_Mode == SAMOVAR_BEER_MODE) {
        ...
        else queue_samovar_command(SAMOVAR_POWER); // Использовать общее переключение мощности
      }
      // ... аналогичная логика для других режимов ...
      else queue_samovar_command(SAMOVAR_POWER); // Переключение питания по умолчанию
    }
...
Эта функция `web_command` получает веб-запрос. Она проверяет аргументы в URL запроса (например, `?start=1`). На основе найденного аргумента она ставит определенное значение команды в очередь `SamovarCommandMsg` через `queue_samovar_command(...)`. Основной цикл логики Samovar извлекает команды из очереди и выполняет соответствующее действие. Это распространенная схема взаимодействия между различными задачами или частями кода во встраиваемых системах.
