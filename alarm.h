...
inline void perform_emergency_stop() {
  ...
  // ЗАКРЫВАЕМ - защита от залива помещения при обрыве шланга/отказе клапана.
  // Уточнено владельцем 19.09.2026: в ректификации, дистилляции, БК и НБК вода и
  // насос работают ещё 3 минуты (mode_arm_emergency_cooling_hold(), mode_common.h),
  // иначе горячая колонна остаётся без охлаждения и пар идёт в помещение. Если
