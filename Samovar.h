...
SemaphoreHandle_t xPendingCommandSemaphore = NULL;
StaticSemaphore_t xPendingCommandSemaphoreBuffer;

SemaphoreHandle_t xLogFileSemaphore = NULL;
StaticSemaphore_t xLogFileSemaphoreBuffer;
...
volatile bool heater_state;                                     // Статус нагрева при затирке/сувиде
...
// Значения SamovarStatusInt — публичный контракт (Lua getState(), см. lua.h).
// ЧИСЛА НЕ МЕНЯТЬ. 1..999 — под-статусы ректификации (см. mode_ops_by_status в mode_registry.h).
constexpr int16_t SAMOVAR_STATUS_IDLE              = 0;    // простой/готов (любой режим)
constexpr int16_t SAMOVAR_STATUS_RECT_WITHDRAWAL   = 10;   // ректификация: идёт отбор
constexpr int16_t SAMOVAR_STATUS_RECT_AUTOPAUSE    = 15;   // ректификация: автопауза между строками программы
constexpr int16_t SAMOVAR_STATUS_RECT_PROGRAM_DONE = 20;   // ректификация: программа выполнена
constexpr int16_t SAMOVAR_STATUS_RECT_CALIBRATION  = 30;   // ректификация: калибровка насоса отбора
constexpr int16_t SAMOVAR_STATUS_PAUSED            = 40;   // ручная пауза (PauseOn), выставляется вне зависимости от режима
constexpr int16_t SAMOVAR_STATUS_RECT_ACCEL        = 50;   // ректификация: разгон колонны
constexpr int16_t SAMOVAR_STATUS_RECT_STABILIZING  = 51;   // ректификация: разгон завершён / стабилизация
constexpr int16_t SAMOVAR_STATUS_RECT_STABLE       = 52;   // ректификация: стабилизация завершена
// Значение играет двойную роль: activeStatus режима "Дистилляция" И граница
// диапазона (< SAMOVAR_STATUS_DISTILLATION — под-статус ректификации). Это
// осознанно, отдельной константы-границы не заводим.
constexpr int16_t SAMOVAR_STATUS_DISTILLATION      = 1000; // activeStatus режима "Дистилляция"
...
// Ректификация:
constexpr int16_t SAMOVAR_STARTVAL_RECT_RUNNING   = 1;    // отбор идёт, есть следующая строка программы
constexpr int16_t SAMOVAR_STARTVAL_RECT_DONE      = 2;    // дошли до конца программы, ждём "Следующая программа"
constexpr int16_t SAMOVAR_STARTVAL_RECT_STOPPING  = 3;    // внутр. однотактный переход к остановке (Menu.ino menu_samovar_start) — не наблюдается извне, транзиент
constexpr int16_t SAMOVAR_STARTVAL_CALIBRATION    = 100;  // калибровка шагового насоса отбора
// Пиво:
constexpr int16_t SAMOVAR_STARTVAL_BEER_START     = 2000; // сессия запущена, ждёт первого перехода программы
constexpr int16_t SAMOVAR_STARTVAL_BEER_HEATING   = 2001; // разогрев до температуры засыпи солода
constexpr int16_t SAMOVAR_STARTVAL_BEER_WAIT_MALT = 2002; // ожидание засыпи солода
// НБК:
...
#ifdef USE_WATER_PUMP
int8_t wp_count;                                                // Переменная для расчета времени работы насоса на повышенной мощности при старте
#endif
